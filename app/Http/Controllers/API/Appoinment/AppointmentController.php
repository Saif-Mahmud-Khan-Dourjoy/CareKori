...
class AppointmentController extends Controller
{
    ...
    public function checkAvailability($provider_unique_user_id, $appointment_date)
    {
        try {
            ...
            $authUser = auth()->user();
            if ($authUser->role && !in_array(strtolower($authUser->role->name), ['customer', 'super admin', 'moderator'])) {
                if ((int)$authUser->id !== (int)$provider->id) {
                    return response()->json(['error' => 'You cannot check availability for this provider.'], 403);
                }
            }
    ...
    {
       $request->merge([
    'new_time' => $request->new_time ?? $request->appointment_time,
]);
...
$request->validate([
    ...
    public function updateConductStatus(Request $request, Appointment $appointment)
    {
        ...
        $validated = $request->validate([
            'conduct_success_status' => 'required',
